...
빗썸 거래소에서 BTC를 자동으로 매매하는 Python 프로그램입니다. 전략 복잡도에 따라 두 가지 버전을 제공합니다.

| 항목 | 기본 버전 (`bithumb_autotrading.py`) | 고급 버전 (`bithumb_autotrading_v2.py`) |
|------|--------------------------------------|------------------------------------------|
| 전략 | 이동평균선(MA5/MA20) 크로스 | MA 크로스 + CryptoPanic 뉴스 감성 분석 |
| AI | 없음 | Claude AI (Anthropic) — 뉴스 감성 판단 |
| 매수 조건 | 골든크로스 | 골든크로스 + 뉴스 긍정 동시 충족 |
| 매도 조건 | 데드크로스 | 데드크로스 + 뉴스 부정 동시 충족 |
| 필요 API 키 | 빗썸 1종 | 빗썸 + Anthropic + CryptoPanic 3종 |
| 실행 환경 | **Docker 컨테이너** (docker compose로 관리) |
| API 키 관리 | `.env` 파일 사용 — 이미지·GitHub에 절대 포함되지 않음 |

신호가 불일치하면 매매를 보류하여 과매매를 방지합니다.
